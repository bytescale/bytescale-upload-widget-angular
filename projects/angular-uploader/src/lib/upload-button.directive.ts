...
export class UploadButtonDirective implements OnInit {
  ...
  @HostListener('click', ["$event"]) onClick(event: any){
    ...
    this.getUploader().open(this.options).then(
      files => {
  ...
  private getUploader(): Uploader {
    if (this.uploader === undefined) {
      throw new Error("[angular-uploader] You must provide the 'uploader' attribute to elements marked with the 'uploadButton' directive.")
    }
